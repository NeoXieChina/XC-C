...
void Add(struct textbook *TextBook, int BookNumber)
{
	int had = HadData(TextBook, BookNumber);
	printf("\n请输入想要添加的图书的ISBN编码：");
	fflush(stdin);
...
void changeItem(struct textbook * TextBook, int id, int length, int type, char info[40], int heads_id)
{
	char *charP;
	unsigned int * intP;
	switch (heads_id) {
	...
	switch (type) {
		case 1: {
			fflush(stdin);
			printf("\n请输入修改后的%s:", info);
			scanf_s("%s", charP,(unsigned)length);
			printf("\n您输入的是：%s", charP);
			printf("\n\n按回车继续...");
			fflush(stdin);
			getchar();
...
void createItem(struct textbook * TextBook, int id, int length, int type, char info[40], int heads_id)
{
	char *charP;
	unsigned int * intP;
	switch (heads_id) {
